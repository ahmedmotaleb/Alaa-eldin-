...
Express + PostgreSQL (عبر `pg`، بدون ORM) منفصل عن الواجهة. الاتصال عبر `DATABASE_URL` (افتراضياً `postgresql://postgres:postgres@localhost:5432/alaa_eldin`)، `server/src/db.ts` يصدّر `pool` (اتصال pg.Pool) و`withTransaction()` (مساعد `BEGIN`/`COMMIT`/`ROLLBACK` حول عميل واحد من الـ pool) بدل `db.transaction()` المتزامنة القديمة. كل الاستعلامات async الآن (`await pool.query(...)`)، placeholders بصيغة `$1, $2, ...` بدل `?`/`@name`، والحصول على id السطر المُدرَج عبر `INSERT ... RETURNING id` بدل `lastInsertRowid`. الجداول: `users`, `sessions`, `password_resets`, `orders`, `order_items`, `categories`, `products`, `discounts`, `stock_movements`, `banners`, `store_settings` (صف واحد ثابت `id=1`، لا يوجد أكثر من متجر), `riders`, `settlements`, `expenses` — كلها تُنشأ بالمخطط النهائي الكامل مباشرة (`CREATE TABLE IF NOT EXISTS`) في `initDb()`، بدون حاجة لأعمدة تُضاف لاحقاً عبر `ALTER TABLE` (ده كان مطلوب مع SQLite القديمة لترقية قاعدة بيانات موجودة، لكن مش لازم هنا لأنها قاعدة بيانات جديدة من الأول). يُزرع `categories`/`products` تلقائياً من `server/src/seedData.ts`، وبانر افتراضي واحد في `banners`، عند أول تشغيل إذا كانت الجداول فارغة. أعمدة `is_admin`/`available`/`bestseller`/`offer`/`active`/`show_todays_offers`/`show_best_sellers`/`cod_enabled` تبقى `INTEGER` (0/1) بدل `BOOLEAN` الحقيقي في Postgres — قرار متعمّد لتقليل التغييرات في كود الـ routes (كل التسلسل `!!row.field` عند القراءة و`field ? 1 : 0` عند الكتابة يشتغل بنفس الشكل).
...
- `GET /api/orders`, `GET /api/orders/:id` — تتطلب جلسة صالحة (`requireAuth`) وتتحقق أن الطلب يخص نفس المستخدم. **`POST /api/orders` لا يتطلب تسجيل دخول** (طلب الزائر مسموح) — لو فيه جلسة صالحة، `user_id` بيتسجّل من `req.user?.id`؛ لو لأ، `user_id = NULL` (العمود `orders.user_id` بقى nullable، بعد ما كان `NOT NULL` قبل دعم الزوار — `db.ts` بيشغّل `ALTER TABLE orders ALTER COLUMN user_id DROP NOT NULL` بشكل idempotent عند كل إقلاع عشان يترقّى أي قاعدة بيانات منشورة قبل كده بالمخطط القديم). يقبل `discountCode` اختياري، ويعيد التحقق منه بنفسه عبر `evaluateDiscount()` (لا يثق بأي مبلغ خصم يرسله المتصفح) قبل إدراج الطلب وزيادة `discounts.used_count`، كل ده داخل نفس معاملة قاعدة البيانات (`withTransaction`)، ويتحقق أيضاً من `cod_enabled` في `store_settings` ويرفض بـ `cod_disabled` لو الدفع عند الاستلام متوقف. `customer.governorate` حقل مطلوب (عمود `customer_governorate` على `orders`). **لا يوجد `PATCH /api/orders/:id/status` للعميل** — كان موجوداً سابقاً ويسمح لأي عميل يغيّر حالة طلبه الخاص لأي قيمة بدون صلاحية إدارية (ثغرة حقيقية تكسر مصداقية "المحفظة")، اتشال نهائياً؛ تغيير الحالة الآن حصراً عبر `PATCH /api/admin/orders/:id/status` (يتطلب `is_admin`). **ومفيش أي endpoint عام يسمح بجلب تفاصيل طلب بدون تسجيل دخول** — حتى الزائر نفسه صاحب الطلب، عشان أي endpoint كده هيسمح لأي حد يعرف رقم الطلب إنه يشوف اسم/موبايل/عنوان عميل تاني. الزائر بيشوف تفاصيل طلبه فقط من رد الإنشاء نفسه (`POST` response) اللي بيتمرر لصفحة التأكيد مباشرة.
- `GET /api/admin/orders` — تتطلب `is_admin`، تستخدم `LEFT JOIN` على `users` (مش `JOIN` عادي) عشان طلبات الزوّار (`user_id IS NULL`) تفضل ظاهرة في القائمة (وبالتالي في التحليلات والمحفظة اللي بتُبنى منها)، مع `accountEmail: null` لو الطلب من زائر بدون حساب.
